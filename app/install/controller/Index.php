...
class Index extends BaseInstall
{
    ...
    public function index()
    {
        ...
        $step = input("step", 1);
        if ($step == 1) {
            return $this->fetch('index/step1', [], $this->replace);
        } else if ($step == 2) {
            //系统变量
            $system_variables = [];
            $phpv = phpversion();
            ...
            return $this->fetch('index/step2', [], $this->replace);
        } else if ($step == 3) {
            return $this->fetch('index/step3', [], $this->replace);
        } else if ($step == 4) {
            set_time_limit(300);
...
//				return $this->returnError([], '平台菜单安装失败！');
    ...
    {
        return $this->fetch('index/step4', [], $this->replace);
    }
    /**
     * 测试数据库
     */
