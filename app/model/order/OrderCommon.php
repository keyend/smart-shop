...
//            $field_list = model("order")->getColumn([["out_trade_no", "=", $item["out_trade_no"]]], "order_id");
     ...
     * @return void
     */
    public function setOrderComplete($order_id = 0)
    {
        $model = model("cron")->getModel();
        $corn = $model->where('relate_id', $order_id)->where('event', 'CronOrderComplete')->find();
        if ($corn) {
            $model->where('id', $corn['id'])->update([
                'execute_time' => 0
            ]);
            ...
            $data['execute_time'] = 0;
            Cache::delete("order_complete_execute_" . $order_id);
            event("CronOrderComplete", $data);
        }
