...
class OrderCreate extends BaseModel
{
     ...
     * @return array
     */
    private function getDeliverStore($shop_goods, $data)
    {
        $result = null;
        $goodsids = array_values(array_column($shop_goods['goods_list'], 'sku_id'));
        $skuCount = (int)model('store_goods_sku')->getModel()->where([
            ['sku_id', 'IN', $goodsids],
            ['store_id', '=', $data['default_store_id']]
        ])->field('id,store_stock')->count();

        if ($skuCount > 0) {
            $result = ['store_id' => $data['default_store_id']];
        }
    ...
    public function shopOrderCalculate($shop_goods, $data)
    {
        ...
        if ($this->is_virtual == 1) {
            ...
            //如果启用分店配送
            // if ($shop_goods["store_config"]['is_use'] == 1) {
            //     // 不是默认门店
            //     if ($data['default_store_id'] > 1) {
            //         $delivery_store = $this->getDeliverStore($shop_goods, $data);
            //         if (!empty($delivery_store)) {
            //             $delivery_store['delivery_type'] = 'store';
            //             $data['delivery'] = $delivery_store;
            //         }
            //     }
            // }
            //查询店铺是否开启外卖配送
