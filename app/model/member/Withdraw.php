...
class Withdraw extends BaseModel
{
    ...
    public function getMemberWithdrawPageList($condition = [], $page = 1, $page_size = PAGE_LIST_ROWS, $order = '', $field = '*')
    {
        ...
        if ($this->export) {
            $excel = app()->make(\extend\Excel::class);
            $options = [
                'title' => '提现申请列表',
                'headers' => [
                    ['field' => 'withdraw_no', 'title' => '流水号', 'width' => 24, 'type' => 'numeric'],
                    ['field' => 'transfer_type_name', 'title' => '提现方式', 'width' => 12],
                    ['field' => 'status_name', 'title' => '提现状态', 'width' => 12],
                    ['field' => 'apply_time', 'title' => '申请时间', 'width' => 24, 'type' => 'date'],
                    ['field' => 'member_id', 'title' => '会员ID', 'width' => 12],
                    ['field' => 'realname', 'title' => '申请人', 'width' => 16],
                    ['field' => 'idnumber', 'title' => '身份证号', 'width' => 24, 'type' => 'numeric'],
                    ['field' => 'mobile', 'title' => '联系电话', 'width' => 18],
                    ['field' => 'bank_name', 'title' => '收款银行', 'width' => 16],
                    ['field' => 'account_number', 'title' => '收款账户', 'width' => 24, 'type' => 'numeric'],
                    ['field' => 'apply_money', 'title' => '申请提现金额', 'width' => 16],
                    ['field' => 'service_money', 'title' => '提现手续费', 'width' => 16],
                    ['field' => 'money', 'title' => '实际转账金额', 'width' => 16],
                    ['field' => 'transfer_time', 'title' => '转账时间', 'width' => 24],
                ]
            ];

            while($page <= $list['page_count']) {
                foreach($list['list'] as &$row) {
                    if (empty($row['bank_name']) && empty($row['account_number'])) {
                        $row['bank_name'] = $row['transfer_type_name'];
                        $row['account_number'] = $row['account_name'];
                    }
                }
                unset($row);

                $excel->export($list['list'], $options);
                $page += 1;
