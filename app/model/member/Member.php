...
class Member extends BaseModel
{
    ...
    public function editMember($data, $condition)
    {
        ...
        if (!isset($info)) {
            $info = model('member')->getInfo($condition);
            $data = array_merge($info, $data);
            event("MemberChange", $data);
        }
     ...
     * @return array
     */
    public function treeMember($member_id = 0)
    {
        $field = 'member_id,source_member,username,nickname,mobile,email,status,headimg,member_level,member_level_name,member_label,member_label_name';
        $member_info = model('member')->getInfo([ [ 'member_id', '=', $member_id ] ], $field);
        $children = $this->treeMemberChildren($member_info['member_id'], 200);
        if ($children) {
     ...
     *
     * @param integer $member_id
     * @param integer $depthment
     * @return array
     */
    private function treeMemberChildren($member_id, $depthment = 9999)
    {
        $result = array();
        if ($depthment > 0) {
            $field = 'member_id,source_member,username,nickname,mobile,email,status,headimg,member_level,member_level_name,member_label,member_label_name';
            $member_list = model('member')->getModel()->where('source_member', $member_id)->field($field)->select()->toArray();
            foreach($member_list as $member_info) {
                $children = $this->treeMemberChildren($member_info['member_id'], $depthment - 1);
                if ($children) {
                    $member_info['children'] = $children;
                }
