...
class Goods extends BaseShop
{
    ...
    public function modifySort()
    {
        if (request()->isAjax()) {
            ...
            $sort = input('sort', 0);
            $this->addLog("修改商品排序", [$goods_id, $sort]);
    ...
    public function offGoods()
    {
        if (request()->isAjax()) {
            ...
            $goods_model = new GoodsModel();
            $this->addLog("商品下架", [$goods_ids, $goods_state]);
    ...
    public function onGoods()
    {
        if (request()->isAjax()) {
            ...
            $goods_state = input("goods_state", 0);
            $goods_model = new GoodsModel();
            $this->addLog("商品上架", [$goods_ids, $goods_state]);
            $res = $goods_model->modifyGoodsState($goods_ids, $goods_state, $this->site_id);
            return $res;
        }
    }
    ...
    public function deleteEvaluate()
    {
        if (request()->isAjax()) {
            ...
            $this->addLog("商品评价删除", $evaluate_id);
    ...
    public function modifyAuditEvaluate()
    {
        if (request()->isAjax()) {
            ...
            $this->addLog("商品评价审核", [$condition, $data]);
            return $res;
        }
    }
    /**
    ...
    public function copyGoods()
    {
        if (request()->isAjax()) {
            ...
            $this->addLog("复制商品:[{$goods_id}]", [$goods_id, $res['data']]);
            return $res;
        }
    }
    /**
