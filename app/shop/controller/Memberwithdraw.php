...
class Memberwithdraw extends BaseShop
{
    ...
    public function lists()
    {
        ...
        $export_flag    = input('export') == '1' ? true : false; //导出

        if (request()->isAjax() || $export_flag == true) {
            $page          = input('page', 1);
            ...
            return $withdraw_model->export($export_flag)->getMemberWithdrawPageList($condition, $page, $page_size, $order);
        } else {
            $this->assign("is_exist", addon_is_exit("memberwithdraw", $this->site_id));
            $pay_model          = new Pay();
            $transfer_type_list = $pay_model->getTransferType($this->site_id);
