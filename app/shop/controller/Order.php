...
class Order extends BaseShop
{
    ...
    {
        // $model = app()->make(\addon\agencyteam\model\AgencyTeamModel::class);
        // $member = $model->where('member_id', 2)->find();
        // if ($member) {
        //     $member->auto_upgrade_team_level(4);
        // }
        // event('OrderComplete', ['order_id' => 7]);
        // die("#!");
...
//                ["order_type", "=", 1],
     ...
     * @return void
     */
    public function complete(OrderCommonModel $model)
    {
        if (request()->isAjax()) {
            $order_id = input("order_id", 0);
            $result = $model->setOrderComplete($order_id);
            return $result;
        }
    }
}