        ...
        if (request()->isAjax()) {
            ...
            if (!empty($list)) {
                foreach ($list as $k => $v) {
                    $v['parent_name'] = '';
                    if ($v['source_member']) {
                        $v['parent_name'] = model('member')->getModel()->where('member_id', $v['source_member'])->value('nickname');
                    }
            ...
            $this->assign('search_text_type', $search_text_type);
...
//			$this->assign('from_type_arr', $from_type_arr['point']);
     ...
     * @return void
     */
    public function treeMember(MemberModel $member_model)
    {
        $member_id = (int)input('id', 0);
        return $member_model->treeMember($member_id);
    }
    ...
    public function change()
    {
        ...
        if(request()->isPost()) {
            ...
            $condition[] = [ 'site_id', '=', $this->site_id ];
            $condition[] = [ 'username|nickname|member_id|realname', 'LIKE', '%' . input('keyword', $member_name) . '%' ];
            $condition[] = [ 'member_id', '<>', $member_id ];
            ...
            return $this->fetch('member/change');
        }
    }
}