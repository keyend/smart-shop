	...
	},
	{
		...
		template: function (orderitem, order) {
			...
			if (order.order_type != 4) {
				h += '<p>';
				h += '<a target="_blank" href="' + ns.url('shop/member/memberlist', {
					'search_text_type': 'nickname',
					'search_text': order.name
				}) + '">' + order.name + '</a>';
				h += '</p>';
	...
	// },
	{
		...
		template: function (orderitem, order) {
			...
			if(order.order_status==4) {
				html += '<a class="layui-btn" href="javascript:orderAction(\'orderComplete\', ' + order.order_id + ')">完成订单</a>';
			}
