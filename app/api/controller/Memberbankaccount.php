...
class Memberbankaccount extends BaseApi
{
    ...
    public function add()
    {
        ...
        $idnumber         = isset($this->params['idnumber']) ? $this->params['idnumber'] : '';// 身份证号
        if (empty($realname)) {
        ...
        if (empty($idnumber)) {
            return $this->response($this->error('', '身份证号不能为空'));
        } else {
            if (!\extend\Idnumber::validateCard($idnumber)) {
                return $this->response($this->error('', '身份证号填写不正确'));
            }
        }
    ...
    public function info()
    {
        ...
        $member_bank_account_model = new MemberBankAccountModel();
        $info                      = $member_bank_account_model->getMemberBankAccountInfo([['member_id', '=', $this->member_id], ['id', '=', $id]], 'id,member_id,realname,idnumber,mobile,withdraw_type,branch_bank_name,bank_account,is_default');
        if (!empty($info['data'])) {
    ...
    public function defaultInfo()
    {
        ...
        if ($token['code'] < 0) return $this->response($token);
        $member_bank_account_model = new MemberBankAccountModel();
        $info                      = $member_bank_account_model->getMemberBankAccountInfo([['member_id', '=', $this->member_id], ['is_default', '=', 1]], 'id,member_id,realname,idnumber,mobile,withdraw_type,branch_bank_name,bank_account,is_default');
        if (!empty($info['data'])) {
            $info['data']['withdraw_type_name'] = $member_bank_account_model->getWithdrawType()[$info['data']['withdraw_type']];
        }
        return $this->response($info);
