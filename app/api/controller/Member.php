...
class Member extends BaseApi
{
    ...
    public function info()
    {
        ...
        if (!empty($info['data'])) {
            ...
            $info['data']['source_member_name'] = '';
            // 插件权益配置
            if (!empty($info['data']['member_level_info'])) {
                $info['data']['member_level_info']['addons'] = json_decode($info['data']['member_level_info']['addons'], true);
            }
            // 推荐人信息
            if ($info['data']['source_member'] != 0) {
                $info['data']['source_member_name'] = model('member')->getValue([['member_id', '=', $info['data']['source_member']]], 'nickname');
            }
        }
    ...
    public function bindmobliecode()
    {
        ...
        if ($exist) {
            ...
            ////
            // return $this->response($this->success(['key' => 'bind_mobile_code_' . md5($code)]));
            ////
    ...
    public function paypwdcode()
    {
        ...
        $code          = str_pad(random_int(1, 9999), 4, 0, STR_PAD_LEFT);// 生成4位随机数，左侧补0
        ///
        // return $this->response($this->success(["key" => 'pay_password_code_' . md5(uniqid(null, true))]));
        ///
        $message_model = new Message();
    ...
    public function modifyusername()
    {
        ...
        $member_model = new MemberModel();
        $info         = $member_model->getMemberInfo([['member_id', '=', $token['data']['member_id'], ['site_id', '=', $this->site_id]]], 'member_id,username,fenxiao_id');
        $res          = null;
        if ($info['data']['username'] != '') {
            ...
            $exist_user = $member_model->getMemberInfo([['username', '=', $username, ['site_id', '=', $this->site_id]]], 'member_id,username');
            if ($exist_user['data']) {
                if ($exist_user['data']['member_id'] != $token['data']['member_id']) {
                    $res = $this->error("", "用户名已存在!");
                }
            }

            if ($res == null) {
                $res = $member_model->editMember(['username' => $username], [['member_id', '=', $token['data']['member_id'], ['site_id', '=', $this->site_id]]]);
                model('fenxiao')->update(['fenxiao_name' => $username], [['fenxiao_id', '=', $info['data']['fenxiao_id']]]);
            }
        }
        return $this->response($res);
    }
    ...
    public function account()
    {
        $token = $this->checkToken();
        if ($token['code'] < 0) return $this->response($token);
        $model = app()->make(\app\model\order\OrderCreate::class);
        $res = app()->make(\app\model\BaseModel::class)->success($model->initMemberAccount($token['data']));
        return $this->response($res);
    }
}