...
class User extends BaseStore
{
	...
	public function index()
	{
		...
		if (request()->isAjax()) {
			...
			$group_id = $group_model->getStoreGroupColumn([ [ "site_id", "=", $this->store_id ], [ "app_module", "=", $this->app_module ] ], 'group_id');
			if (!empty($group_id['data'])) {
				$condition[] = [ 'group_id', 'in', $group_id['data'] ];
			}
	...
	public function addUser()
	{
		if (request()->isAjax()) {
			...
			$result = $user_model->addUser($data, $this->store_id);
			return $result;
		} else {
			$group_model = new Group();
	...
	public function editUser()
	{
		...
		if (request()->isAjax()) {
			...
			$result = $user_model->editUser($data, $condition, $this->store_id);
			return $result;
		} else {
			$uid = input("uid", 0);
			$this->assign("uid", $uid);
