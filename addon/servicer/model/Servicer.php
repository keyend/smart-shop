...
//        $group = 'sm.member_id';
//
//        return model('servicer')->getList($condition, $field, 'chat_count asc', $alias, $join, $group);
        $condition = [['online', '=', 1], ['shop_id', '=', $site_id]];
        $list = model('servicer')->getList($condition, '*');
        foreach($list as $k => $v){
            $item_condition = array(
                ['servicer_id', '=', $v['user_id']],
                ['online', '=', 1]//在线
            );
            $item_count = model('servicer_member')->getCount($item_condition);
            $list[$k]['chat_count'] = $item_count;//正在聊天的会员数
        }
