...
class Chat extends BaseServicer
{
    ...
    public function answer()
    {
        ...
        if ($contentType == Dialogue::CONTENTTYPE_GOODSKU) {
            $data = json_decode($servicerSay, true);
            $goodsId = $this->getGoodsSkuId($data);
            $response['goods'] = ['goods_sku_id' => $goodsId];
            $servicerSay = "";
        }
        ...
        return $this->result($response, 1, '发送成功');
    }
    /**
     * 客服下线
     * @return array|void
     ...
     * @return void
     */
    private function getGoodsSkuId($message) {
        if (!empty($message)) {
            if (isset($message['selected_sku_list'])) {
                $goods_model = app()->make(Goods::class);
                foreach($message['selected_sku_list'] as $sku) {
                    return $sku['sku_id'];
                }
            }
        }
