...
class Login extends Controller
{
    ...
    public function login()
    {
        ...
        if (request()->isAjax()) {
            ...
            if ($result['code'] == 0) {
                $userInfo = Session::get($this->app_module . "_" . $this->site_id . "." . "user_info");
            }
            if (empty($userInfo)) {
            ...
            $this->assign("shop_info", ['site_name' => "客服中心"]);
            //平台配置信息
            ...
            $shop_model = new Shop();
            $shop_info = $shop_model->getShopInfo($site_condition)['data'] ?? [];
//            dump(array_merge($site_info, $shop_info));exit();
            $this->assign('website_info', array_merge($site_info, $shop_info));
