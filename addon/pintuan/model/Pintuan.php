...
class Pintuan extends BaseModel
{
    ...
    public function addPintuan($pintuan_data, $sku_list)
    {
        ...
        try {
            ...
            if ($pintuan_data[ 'status' ] == 1) {
                $goods = new Goods();
                $goods->modifyPromotionAddon($pintuan_data[ 'goods_id' ], [ 'pintuan' => $pintuan_id ]);
    ...
    {
        $field = 'p.*,g.goods_name,g.goods_image,g.goods_stock';
        $alias = 'p';
        $join = [
            [
                'goods g',
     ...
     * @param string $field
     */
    public function getPintuanGoodsPageList($condition = [], $page = 1, $page_size = PAGE_LIST_ROWS, $order = 'pp.is_recommend desc,pp.start_time desc', $field = 'pp.site_id,pp.pintuan_name,pp.is_virtual_goods,pp.pintuan_num,pp.pintuan_time,pp.is_recommend,pp.group_num,pp.order_num,ppg.id,ppg.pintuan_id,ppg.pintuan_price,ppg.promotion_price,sku.sku_id,sku.price,g.goods_id,g.goods_name,g.goods_image,(sku.sale_num + sku.virtual_sale) as sale_num,sku.unit')
    {
        $alias = 'pp';
        $join = [
            [ 'promotion_pintuan_goods ppg', 'pp.pintuan_id = ppg.pintuan_id and pp.goods_id =ppg.goods_id', 'inner' ],
            [ 'goods g', 'pp.goods_id = g.goods_id', 'inner' ],
            [ 'goods_sku sku', 'g.sku_id = sku.sku_id', 'left' ],
        ];
        $group = 'pp.goods_id';
        $list = model('promotion_pintuan')->pageList($condition, $field, $order, $page, $page_size, $alias, $join, $group);
        return $this->success($list);
    }
    /**
     * 开启拼团活动
