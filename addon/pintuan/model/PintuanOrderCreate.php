...
//                    }
    ...
    public function calculate($data)
    {
        ...
        $pintuan_info_result = $pintuan_model->getPintuanInfo([["pintuan_id", "=", $pintuan_id], ['site_id', '=', $data['site_id']]]);
        ...
        if ($pintuan_info["buy_num"] < $data['num'] && $pintuan_info["buy_num"] > 0) {
            $this->error     = 1;
            $this->error_msg = "该商品限制购买不能大于" . $pintuan_info["buy_num"] . "件!";
        }
        $data["pintuan_info"] = $pintuan_info;
        //查询拼团组信息
        $pintuan_model             = new PintuanGroup();
        $group_id                  = $data["group_id"];
        $pintuan_group_info_result = $pintuan_model->getPintuanGroupInfo([["group_id", "=", $group_id], ['site_id', '=', $data['site_id']]]);
...
//        Cache::tag("order_create_pintuan_".$data['member_id'])->set("order_create_pintuan_".$data['id'].'_'.$data['num'].'_'.$data['member_id'], $shop_goods_list, 600);
    ...
    public function getPintuanGoodsInfo($id, $data)
    {
        ...
        $info  = model("promotion_pintuan_goods")->getInfo([['nppg.id', '=', $id], ['nppg.site_id', '=', $data['site_id']]], $field, $alias, $join);
//        $goods_list = model("promotion_pintuan_goods")->getList([['nppg.id', '=', $id]], $field, '', $alias, $join);
        $shop_goods_list = [];
        if (!empty($info)) {
            //判断是否是虚拟订单
            if ($info['is_virtual']) {
