...
class Bargain extends BaseModel
{
     ...
     * @param $member_id
     */
    public function launch($id, $member_id, $site_id)
    {
        $join = [
            [ 'goods_sku sku', 'pbg.sku_id = sku.sku_id', 'inner' ],
        ];
        $bargain_info = model('promotion_bargain_goods')->getInfo([ [ 'pbg.id', '=', $id ], [ 'pbg.site_id', '=', $site_id ], [ 'pbg.status', '=', 1 ] ], 'pbg.*, sku.sku_image,sku.sku_name,sku.price', 'pbg', $join);
    ...
    {
        $launch_info = model('promotion_bargain_launch')->getInfo([ [ 'launch_id', '=', $launch_id ], [ 'site_id', '=', $site_id ] ]);
