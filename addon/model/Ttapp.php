...
class Ttapp extends BaseModel
{
    ...
    public function authCodeToOpenid($param)
    {
        try {
            ...
            if ($result['code'] == 'SUCCESS') {
                $member = model('member')->getInfo([['toutiao_openid', '=', $result['data']['openid']]], 'member_id');
                if (!empty($member)) {
                    $result['data'] = array_merge($result['data'], $member);
                }
            }
    ...
    {
        $source_file_path = $this->createTempPackage($site_id, 'public/ttapp');
        $file_arr         = getFileMap($source_file_path);
        if (!empty($file_arr)) {
            $zipname = 'upload/ttapp_' . $site_id . '_' . date('Ymd') . '.zip';
            $zip = new \ZipArchive();
