...
class MemberRechargeCard extends BaseModel
{
    ...
    public function addMemberAccount($order_info)
    {
        ...
        //修改用户的余额
        $member_account->addMemberAccount($order_info['site_id'], $order_info['member_id'], 'balance', $order_info['face_value'], 'memberrecharge', '0', '会员充值增加余额');
        //积分
        if ($order_info['point'] > 0) {
            $member_account->addMemberAccount($order_info['site_id'], $order_info['member_id'], 'point', $order_info['point'], 'memberrecharge', '0', '会员充值增加积分');
        }
        //成长值
        if ($order_info['growth'] > 0) {
            $member_account->addMemberAccount($order_info['site_id'], $order_info['member_id'], 'growth', $order_info['growth'], 'memberrecharge', '0', '会员充值增加成长值');
        }
        //添加优惠券
        if (!empty($order_info['coupon_id'])) {
            ...
            foreach ($coupon_id as $v) {
                ...
                $coupon_model->receiveCoupon($v, $coupon_type['data']['site_id'], $order_info['member_id'], 1);
            }
        }
    }
    /**
