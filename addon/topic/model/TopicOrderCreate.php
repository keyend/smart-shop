...
//                    }
    ...
    public function calculate($data)
    {
        ...
        $topic_info_result = $topic_model->getTopicInfo([["topic_id", "=", $topic_id], ['site_id', '=', $data['site_id']]]);
...
//            return $cache;
//        }
        $goods_list                    = $this->getTopicGoodsInfo($data["id"], $data['num'], $data);
...
//        Cache::tag("order_create_topic_".$data['member_id'])->set("order_create_topic_".$data['id'].'_'.$data['num'].'_'.$data['member_id'], $shop_goods_list, 600);
     ...
     * @param unknown $bl_id
     */
    public function getTopicGoodsInfo($id, $num, $data)
    {
        //组装商品列表
        ...
        $info            = model("promotion_topic_goods")->getInfo([['ptg.id', '=', $id], ['ptg.site_id', '=', $data['site_id']]], $field, $alias, $join);
        $shop_goods_list = [];
        if (!empty($info)) {
            //判断是否是虚拟订单
            if ($info['is_virtual']) {
